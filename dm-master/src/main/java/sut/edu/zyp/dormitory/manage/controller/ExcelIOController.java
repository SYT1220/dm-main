...
public class ExcelIOController {
    ...
    private List<Map<String, String>> getDataByUrl(String url) {
        ...
        if ("/admin".equals(url)) {
            ...
            data = addBuildingDataWithDormitoryManagerName(buildingEntities.iterator(), data);
            ...
            data = addDormitoryDataWithBuildingName(dormitoryEntities.iterator(), data);
        } else if ("/live".equals(url)) {
            Iterable<LiveEntity> liveEntities = liveRepository.findAll();
            data = addLiveDataWithStudentInfo(liveEntities.iterator(), data);
    ...
    private RestDataExcelExportRequest GenByUrl(String url) {
        ...
        if ("/admin".equals(url)) {
            String[] columns = {"name", "password", "createTime", "updateTime"};
            String[] columnNames = {"账号", "密码", "创建时间", "更新时间"};
            ...
            restDataExcelExportRequest.setSheetName("admin");
            restDataExcelExportRequest.setUrl("/admin");
        } else if ("/building".equals(url)) {
            String[] columns = {"name", "location", "dormitoryManagerId", "createTime", "updateTime"};
            String[] columnNames = {"楼宇名称", "位置", "辅导员编号", "创建时间", "更新时间"};
            restDataExcelExportRequest.setColumns(Arrays.asList(columns));
            restDataExcelExportRequest.setColumnNames(Arrays.asList(columnNames));
            restDataExcelExportRequest.setExcelName("数据导出 - 楼宇");
            restDataExcelExportRequest.setSheetName("building");
            restDataExcelExportRequest.setUrl("/building");
        } else if ("/dormitoryManager".equals(url)) {
            String[] columns = {"sn", "name", "password", "sex", "createTime", "updateTime"};
            String[] columnNames = {"辅导员编号", "姓名", "密码", "性别", "创建时间", "更新时间"};
            restDataExcelExportRequest.setColumns(Arrays.asList(columns));
            restDataExcelExportRequest.setColumnNames(Arrays.asList(columnNames));
            restDataExcelExportRequest.setExcelName("数据导出 - 辅导员");
            restDataExcelExportRequest.setSheetName("dormitoryManager");
            restDataExcelExportRequest.setUrl("/dormitoryManager");
        } else if ("/dormitory".equals(url)) {
            String[] columns = {"sn", "buildingId", "floor", "maxNumber", "livedNumber", "createTime", "updateTime"};
            String[] columnNames = {"宿舍编号", "所属楼宇", "楼层", "最大可住人数", "已住人数", "创建时间", "更新时间"};
            restDataExcelExportRequest.setColumns(Arrays.asList(columns));
            restDataExcelExportRequest.setColumnNames(Arrays.asList(columnNames));
            restDataExcelExportRequest.setExcelName("数据导出 - 宿舍");
            restDataExcelExportRequest.setSheetName("dormitory");
            restDataExcelExportRequest.setUrl("/dormitory");
        } else if ("/live".equals(url)) {
            String[] columns = {"studentId", "studentName", "dormitoryId", "liveDate", "createTime", "updateTime"};
            String[] columnNames = {"学生编号", "姓名", "宿舍编号", "入住日期", "创建时间", "更新时间"};
            restDataExcelExportRequest.setColumns(Arrays.asList(columns));
            restDataExcelExportRequest.setColumnNames(Arrays.asList(columnNames));
            restDataExcelExportRequest.setExcelName("数据导出 - 入住");
            restDataExcelExportRequest.setSheetName("live");
            restDataExcelExportRequest.setUrl("/live");
        } else if ("/student".equals(url)) {
            String[] columns = {"sn", "name", "password", "sex", "createTime", "updateTime"};
            String[] columnNames = {"学生编号", "姓名", "密码", "性别", "创建时间", "更新时间"};
            restDataExcelExportRequest.setColumns(Arrays.asList(columns));
            restDataExcelExportRequest.setColumnNames(Arrays.asList(columnNames));
            restDataExcelExportRequest.setExcelName("数据导出 - 学生");
            restDataExcelExportRequest.setSheetName("student");
        ...
        try {
            if (file.isEmpty()) {
                ...
                for (int column = 0; column < sheet.getColumns(); column++) {
                    columns.add(sheet.getCell(column, 0).getContents());
                }
                //第2行至末尾，数据行，中间不允许空行
                List<Map<String, String>> data = new ArrayList<>();
    ...
    private List<Map<String, String>> addBuildingDataWithDormitoryManagerName(Iterator iterator, List<Map<String, String>> data) {
        while (iterator.hasNext()) {
            BuildingEntity building = (BuildingEntity) iterator.next();
            Map<String, String> row = (Map<String, String>) JSON.parseObject(JSON.toJSONString(building), Map.class);
            
            if (building.getDormitoryManagerId() != null && !building.getDormitoryManagerId().isEmpty()) {
                DormitoryManagerEntity dormitoryManager = dormitoryManagerRepository.findById(building.getDormitoryManagerId()).orElse(null);
                if (dormitoryManager != null) {
                    row.put("dormitoryManagerId", dormitoryManager.getSn());
                }
            }
    ...
    private List<Map<String, String>> addDormitoryDataWithBuildingName(Iterator iterator, List<Map<String, String>> data) {
        while (iterator.hasNext()) {
            DormitoryEntity dormitory = (DormitoryEntity) iterator.next();
            Map<String, String> row = (Map<String, String>) JSON.parseObject(JSON.toJSONString(dormitory), Map.class);
            
            // 根据 buildingId 查询楼宇信息
            if (dormitory.getBuildingId() != null && !dormitory.getBuildingId().isEmpty()) {
                BuildingEntity building = buildingRepository.findById(dormitory.getBuildingId()).orElse(null);
                if (building != null) {
                    row.put("buildingId", building.getName());
                }
            }
    ...
    private List<Map<String, String>> addLiveDataWithStudentInfo(Iterator iterator, List<Map<String, String>> data) {
        while (iterator.hasNext()) {
            LiveEntity live = (LiveEntity) iterator.next();
            Map<String, String> row = (Map<String, String>) JSON.parseObject(JSON.toJSONString(live), Map.class);
            
            // 根据 studentId 查询学生信息
            if (live.getStudentId() != null && !live.getStudentId().isEmpty()) {
                StudentEntity student = studentRepository.findById(live.getStudentId()).orElse(null);
                if (student != null) {
                    row.put("studentId", student.getSn());
                    row.put("studentName", student.getName());
                }
            }
            
            // 根据 dormitoryId 查询宿舍信息
            if (live.getDormitoryId() != null && !live.getDormitoryId().isEmpty()) {
                DormitoryEntity dormitory = dormitoryRepository.findById(live.getDormitoryId()).orElse(null);
                if (dormitory != null) {
                    row.put("dormitoryId", dormitory.getSn());
                }
            }
